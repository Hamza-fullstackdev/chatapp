...
const BG_COLORS = ['#25D366', '#1E6FD9', '#E5423D', '#F3A132', '#5F59C6', '#0E8A3A', '#111B21', '#7B8794'];
...
export default function CreateStatusScreen() {
  ...
  const [mode, setMode] = useState<'text' | 'media'>(params.mode === 'media' ? 'media' : 'text');
  const [text, setText] = useState('');
  const [bg, setBg] = useState(BG_COLORS[Math.floor(Math.random() * BG_COLORS.length)]!);
  ...
  const pickMedia = async () => {
    ...
    try {
      ...
      if (!permission.granted) {
        Alert.alert('Permission required', 'Allow photo library access to add media statuses.');
        return;
      }
  ...
  const post = async () => {
    ...
    if (mode === 'media' && !picked) {
      Alert.alert('Add media', 'Choose an image or video to share as a status.');
      return;
    }
    setPosting(true);
    try {
      ...
      const { status } = await statusesApi.create({
        ...
        mimeType,
        audience,
        excludeUserIds: audience === 'my_contacts_except' ? excludeUserIds : undefined,
        includeUserIds: audience === 'only_share_with' ? includeUserIds : undefined,
      });
  ...
  if (mode === 'media' && picking && !picked) {
    return (
      <View style={[styles.safe, { backgroundColor: '#000000', justifyContent: 'center' }]}>
  ...
  return (
    <KeyboardAvoidingView
      style={[styles.safe, { backgroundColor: mode === 'text' ? bg : '#000000' }]}
      ...
      <View style={styles.content}>
        {mode === 'text' ? (
          ...
          <View style={styles.mediaContainer}>
            {picked.contentType.startsWith('video/') ? (
              <View style={styles.videoPlaceholder}>
                ...
                <Text style={{ color: '#FFFFFF', marginTop: 12, fontSize: 16 }}>Video status</Text>
              </View>
            ) : (
              <Image source={{ uri: picked.uri }} style={styles.mediaPreview} contentFit="contain" />
            )}
            <TextInput
              value={text}
              onChangeText={setText}
          ...
          <Pressable style={styles.emptyMedia} onPress={() => void pickMedia()}>
            ...
            <Text style={{ color: '#FFFFFF', marginTop: 12 }}>Tap to add media</Text>
          </Pressable>
        )}
      </View>
